...
public class ServicesController {
	...
	public boolean updateScoreFromTaker(@ModelAttribute Contract contract, int coinAmount, int giverNo,
		...
		boolean result = servicesService.updateScoreFromTaker(contract, reviewContent, userNo, coinAmount, giverNo);
	...
	@ResponseBody
	public boolean updateScoreFromGiver(@ModelAttribute Contract contract, @RequestParam int coinAmount,
			@RequestParam int takerNo, HttpSession session) {
		...
		boolean result = servicesService.updateScoreFromGiver(contract, userNo, coinAmount, takerNo);
	...
	public String registerServicePhoto(@RequestParam(required = false) Integer width,
		 ...
		 * md.update(ext.getBytes()); byte[] digest = md.digest(); String myHash =
	...
	public List<Card> ajaxSearchService(Service service, BindingResult bindingResult, Date serviceDate, int term,
		...
		System.out.println("서비스일 : " + serviceDate);
