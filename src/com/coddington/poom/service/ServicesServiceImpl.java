		...
		for (Card likeServiceCard : likeServiceCardList) {
			...
			for (Schedule schedule : schedules) {
				...
				if (schedule.getType() == "repeat") {
					...
					// String형(ex:mon10) serviceDay에서 요일 String 얻어옴
					String dayNameFromServiceDay = schedule.getServiceDay().substring(0, 3);// mon, tue, wed...
					int dayNumFromServiceDay = 0; // serviceDay로부터 얻은 요일의 index 넘버
					...
					if (dayDiff >= 49) {
						...
						scheduleTotalCount += (dayDiff / 7) + 1;// 예를 들면 일 수 차이가 14~20일이면 1개의 반복 스케쥴이 3개의 일정이 된다.
																// 0~6일이면 1개, 7~13일이면 2개, 즉, 일 수 차이를 7로 나눈 몫 + 1개의 일정이
																// 된다.
					} // if~else end
					...
					boolean isAfterToday = calendarForServiceDate.after(calendarForNow);// 단발성 스케쥴이 오늘 이후인지
					boolean isBeforeMaxDate = calendarForServiceDate.before(calendarForMaxDate);// 단발성 스케쥴이 56일 이후보다
																								// 이전인지
