...
$calendarWrap
		.on(
				...
				function(evt) {
					...
					var selectedDate = $(this).attr("data-schedule").slice(0,
							-2), selectedHour = $(this).text();
					...
					if (!($(this).hasClass("selected"))) {
						...
						var selectedScheduleObject = {
							...
							selectedMoment : moment($(this).attr(
									"data-schedule")),
						...
						if (selectedScheduleObjectArr.length < 10) {
							...
							selectedScheduleObjectArr
									.push(selectedScheduleObject);
							...
							selectedScheduleObjectArr
									.sort(function(a, b) {
										...
										return a.selectedMoment < b.selectedMoment ? -1
												: a.selectedMoment > b.selectedMoment ? 1
														: 0;
									});
...
$sendContractBtn.click(function() {
	...
	_.each(selectedScheduleObjectArr, function(schedule) {
		scheduleElement += "<span>"
				+ schedule.expression.replace("선택 일정 :", "") + "</span>";
	});
...
$detailsSendContractBtn.click(function() {
	...
	_.each(selectedScheduleObjectArr, function(selectedSchedule) {		
		contractSchedules.push({ "serviceDate" : moment(selectedSchedule.selectedMoment._i).unix(), "scheduleNo" : selectedSchedule.scheduleNo});
	});
